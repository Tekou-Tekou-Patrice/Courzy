...
class LoginChef extends StatefulWidget {
  ...
  const LoginChef({super.key,required this.onTap});
...
class _LoginChefState extends State<LoginChef> {
  ...
  Future<void> loginChefDepart() async {
    final url = Uri.parse('http://localhost:8084/loginchef');
    ...
    try {
      ...
      if (response.statusCode == 200) {Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => DashboardChef()),
        );
      } else {
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      ...
                      GestureDetector(
                        onTap: widget.onTap,
                        child: Text('S\'inscrire',style: TextStyle(color: Color.fromARGB(255, 193, 55, 218)),)),
